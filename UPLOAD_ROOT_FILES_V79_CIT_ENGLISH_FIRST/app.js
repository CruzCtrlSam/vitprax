  ...
  function renderTrackSwitcher() {
    ...
    els.certificationTrackGrid.innerHTML = tracks.map((track) => {
      ...
      const detail = track.blueprint
        ? `${t("blueprint")}: ${track.blueprint} · ${track.domains?.filter((domain) => !domain.supplemental).length || 0} ${t("weightedDomains")}`
        : `${track.questionTarget || questionBank.length} ${prefs.language === "es" ? "preguntas" : "questions"}`;
      return `
