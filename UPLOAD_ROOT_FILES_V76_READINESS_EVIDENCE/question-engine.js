(function (root, factory) {
  ...
  function inferDifficulty(question, style) {
    ...
    if (value.includes("easy") || value.includes("foundational")) return 1;
    if (value.includes("intermediate")) return 2;
    if (value.includes("moder")) return 3;
    if (value.includes("exam-level") || value.includes("exam level")) return 4;
  ...
  function buildQuestionSession(inputQuestions, options = {}) {
    ...
    const mode = ["exam", "diagnostic", "forge"].includes(options.mode) ? options.mode : "practice";
    ...
    if (asArray(options.domainWeights).length && (mode === "exam" || mode === "diagnostic")) {
      return buildWeightedQuestionSession(normalized, { ...options, count: target, language, random });
    }
  ...
  function buildWeightedQuestionSession(inputQuestions, options = {}) {
    const random = options.random || Math.random;
    const normalized = normalizeQuestions(inputQuestions, { language: options.language || "en", certificationId: options.certificationId });
    const weightedDomains = asArray(options.domainWeights).filter((domain) => Number(domain.weight) > 0 && !domain.supplemental);
    const totalWeight = weightedDomains.reduce((sum, domain) => sum + Number(domain.weight || 0), 0) || 1;
    const count = Math.min(Number(options.count || 10), normalized.length);
    const raw = weightedDomains.map((domain) => {
      const exact = (Number(domain.weight || 0) / totalWeight) * count;
      return { ...domain, exact, count: Math.floor(exact), remainder: exact - Math.floor(exact) };
    });
    let used = raw.reduce((sum, domain) => sum + domain.count, 0);
    raw.sort((a, b) => b.remainder - a.remainder);
    for (let index = 0; used < count && raw.length; index += 1) {
      raw[index % raw.length].count += 1;
      used += 1;
    }
    ...
    const domainCounts = {};
    raw.forEach((domain) => {
      const group = normalized.filter((question) => question.domain === domain.id || question.topic === domain.id || question.metadata.topicId === domain.id);
      const picks = pickFromGroup(group, domain.count, options.progress || {}, usedIds, usedFamilies, random);
      selected.push(...picks);
      domainCounts[domain.id] = picks.length;
    });
    if (selected.length < count) {
      selected.push(...pickFromGroup(normalized, count - selected.length, options.progress || {}, usedIds, usedFamilies, random));
    }
    return {
      questions: shuffle(selected, random).slice(0, count),
      diagnostics: {
        requested: Number(options.count || 10),
        returned: Math.min(count, selected.length),
        available: normalized.length,
        domainCounts,
        blueprintWeighted: true
      }
    };
  }

  function validateQuestionBatch(inputQuestions, options = {}) {
    ...
    questions.forEach((question, index) => {
      if (!question.id) errors.push(`Question ${index + 1} is missing an id.`);
      if (ids.has(question.id)) errors.push(`Duplicate question id: ${question.id}`);
      ids.add(question.id);
      if (!question.metadata.certificationId) errors.push(`${question.id} is missing certificationId.`);
      if (!question.domain && !question.topic && !question.metadata.topicId) errors.push(`${question.id} is missing a domain/topic.`);
      const answers = answerList(question, options.language || "en");
      if (answers.length !== 4) warnings.push(`${question.id} has ${answers.length} answer choices; expected 4.`);
      if (!answers.some((answer) => answer.id === question.correctAnswerId || answer.id === question.correctAnswer)) errors.push(`${question.id} does not include its correct answer in the answer choices.`);
      const content = contentFor(question, options.language || "en");
      if (!content.explanation && !content.explanationSections?.whyCorrect) warnings.push(`${question.id} is missing an instructor explanation.`);
      if (!question.incorrectAnswerExplanations && !content.explanationSections?.whyOthersWrong?.length) warnings.push(`${question.id} is missing distractor explanations.`);
    });
    return { valid: errors.length === 0, errors, warnings, total: questions.length };
  }
