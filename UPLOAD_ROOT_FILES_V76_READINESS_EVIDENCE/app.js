(function () {
  ...
  function examHistory() {
    return (progress.history || []).filter((entry) => entry.mode === "exam" && Number(entry.total) >= 50);
  }
  ...
  function readinessModel() {
    ...
    const weightedStats = stats.filter((item) => !activeTrack().domains?.find((domain) => domain.id === item.topic)?.supplemental);
    ...
    const fullExamTaken = (progress.history || []).some((entry) => entry.mode === "exam" && Number(entry.total) >= 100);
    ...
    const expectedStyles = new Set(questionBank.map((question) => question.metadata?.style || question.questionType).filter(Boolean));
    const expectedDifficulties = new Set(questionBank.map((question) => String(question.metadata?.difficulty || question.difficulty || "")).filter(Boolean));
