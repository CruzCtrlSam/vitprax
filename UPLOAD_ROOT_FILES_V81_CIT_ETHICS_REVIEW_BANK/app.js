(function () {
  ...
  const DEFAULT_CERTIFICATION_ID = CONFIG.certificationId || "texas-life";
  const TRACKS = window.VITPRAX_CERTIFICATIONS || {};
  ...
  const text = {
    en: {
      ...
      trackSwitcherDesc: "Keep each exam track, progress record, and practice history separate.",
      currentTrack: "Current track",
      activeTrack: "Active",
      switchTrack: "Switch track",
      trackTexasDesc: "Texas Life Insurance exam prep with bilingual study, practice, exam mode, flashcards, and saved progress.",
      trackCitDesc: "BCSP CIT architecture with domains, blueprint weights, Forge-style practice, flashcards, and sample original questions.",
      citEnglishFirst: "CIT practice is English-first to match exam conditions. Use Spanish mode for navigation, study support, and bilingual review.",
      blueprint: "Blueprint",
      weightedDomains: "Weighted domains",
      practiceQuestions: "practice questions",
      officialQuestionFormat: "question official-style exam",
      supplemental: "Supplemental",
      vitpraxReadiness: "Vitprax Readiness",
      domain: "Domain",
      ...
      coldDiagnostic: "50-question cold diagnostic",
      forgePractice: "Forge practice",
      darkMode: "Dark mode",
      ...
      readinessEstimate: "Readiness estimate",
      ...
      readinessNoteTexas: "Vitprax estimates readiness from your practice history. It cannot guarantee an official exam result.",
      readinessNoteCit: "BCSP does not publish readiness as a simple percent cutoff here. Vitprax estimates readiness from coverage, accuracy, weak spots, and exam-style practice.",
      ...
      takeDiagnostic: "Take a 50-question diagnostic",
      ...
      baselineGoal: "Answer at least 50 questions so Vitprax can build your baseline.",
      ...
      finishEarlyConfirm: "Finish this session now? Your current answers will be scored.",
      ...
      strongPracticeSet: "Strong practice set",
      practiceSetComplete: "Practice set complete",
      keepBuilding: "Keep building",
      limitedPracticeSet: "Limited practice set",
      setBreakdown: "Breakdown for this set",
      limitedSample: "Limited sample",
      supplementalTopic: "Supplemental",
    ...
    es: {
      subTitle: "Forge Your Future.",
      chooseCertification: "Elige tu certificación",
      trackSwitcherDesc: "Mantén cada examen, progreso e historial de práctica por separado.",
      currentTrack: "Ruta actual",
      activeTrack: "Activa",
      switchTrack: "Cambiar ruta",
      trackTexasDesc: "Preparación para Texas Life Insurance con estudio bilingüe, práctica, simulador, tarjetas y progreso guardado.",
      trackCitDesc: "Arquitectura BCSP CIT con dominios, pesos del blueprint, práctica estilo Forge, tarjetas y preguntas originales de muestra.",
      citEnglishFirst: "La práctica CIT se mantiene en inglés para parecerse al examen. Usa español para navegación, apoyo de estudio y repaso bilingüe.",
      blueprint: "Blueprint",
      weightedDomains: "Dominios ponderados",
      practiceQuestions: "preguntas de práctica",
      officialQuestionFormat: "preguntas en formato oficial",
      supplemental: "Suplementario",
      vitpraxReadiness: "Preparación Vitprax",
      domain: "Dominio",
      ...
      allQuestionTypes: "Todos los tipos",
      coldDiagnostic: "Diagnóstico inicial de 50 preguntas",
      forgePractice: "Práctica Forge",
      ...
      readinessEstimate: "Estimación de preparación",
      readinessConfidence: "Confianza del cálculo",
      lowConfidence: "Baja",
      mediumConfidence: "Media",
      highConfidence: "Alta",
      lastScore: "Último resultado",
      noAttemptsYet: "Sin intentos todavía",
      readinessNoteTexas: "Vitprax estima tu preparación según tu historial de práctica. No puede garantizar un resultado oficial.",
      readinessNoteCit: "BCSP no presenta la preparación como un porcentaje público simple. Vitprax estima tu preparación según cobertura, precisión, puntos débiles y práctica estilo examen.",
      ...
      takeDiagnostic: "Haz un diagnóstico de 50 preguntas",
      ...
      baselineGoal: "Contesta al menos 50 preguntas para que Vitprax cree tu base.",
      ...
      finishEarlyConfirm: "¿Finalizar esta sesión ahora? Tus respuestas actuales serán calificadas.",
      ...
      strongPracticeSet: "Buena práctica",
      practiceSetComplete: "Práctica completada",
      keepBuilding: "Sigue construyendo",
      limitedPracticeSet: "Práctica limitada",
      setBreakdown: "Desglose de esta práctica",
      limitedSample: "Muestra limitada",
      supplementalTopic: "Suplementario",
  ...
  const els = {
    ...
    certificationTrackGrid: document.getElementById("certificationTrackGrid"),
    ...
    subtopicFilterWrap: document.getElementById("subtopicFilterWrap"),
    subtopic: document.getElementById("subtopicSelect"),
    difficultyFilterWrap: document.getElementById("difficultyFilterWrap"),
    difficulty: document.getElementById("difficultySelect"),
    questionTypeFilterWrap: document.getElementById("questionTypeFilterWrap"),
    questionType: document.getElementById("questionTypeSelect"),
    ...
    missionReadinessNote: document.getElementById("missionReadinessNote"),
  ...
  let prefs = loadJson(PREF_KEY, { language: "en", theme: "light", certificationId: DEFAULT_CERTIFICATION_ID });
  prefs.certificationId = prefs.certificationId || DEFAULT_CERTIFICATION_ID;
  let progress = defaultProgress();
  let questionBank = normalizeQuestionBank(localQuestionSource());
  let flashcardBank = localFlashcardSource();
  let session = loadJson(sessionKey(), null);
  ...
  function activeCertificationId() {
    const requested = prefs?.certificationId || DEFAULT_CERTIFICATION_ID;
    return TRACKS[requested] ? requested : DEFAULT_CERTIFICATION_ID;
  }

  function activeTrack() {
    return TRACKS[activeCertificationId()] || TRACKS["texas-life"] || {
      id: "texas-life",
  ...
  function isTexasTrack() {
    return activeCertificationId() === "texas-life";
  }

  function activeStudy() {
    const track = activeTrack();
    if (track.study) return track.study;
    if (track.studyGlobal && window[track.studyGlobal]) return window[track.studyGlobal];
    return window.CERTIVO_STUDY || { chapters: [], concepts: [] };
  }

  function localQuestionSource() {
    const track = activeTrack();
    if (Array.isArray(track.questions)) return [...track.questions];
    if (track.questionGlobal && Array.isArray(window[track.questionGlobal])) return [...window[track.questionGlobal]];
    return Array.isArray(window.CERTIVO_QUESTIONS) ? [...window.CERTIVO_QUESTIONS] : [];
  }

  function questionSourceForTrack(track) {
    if (Array.isArray(track?.questions)) return [...track.questions];
    if (track?.questionGlobal && Array.isArray(window[track.questionGlobal])) return [...window[track.questionGlobal]];
    return Array.isArray(window.CERTIVO_QUESTIONS) ? [...window.CERTIVO_QUESTIONS] : [];
  }

  function localFlashcardSource() {
    const study = activeStudy();
    return Array.isArray(study?.concepts) ? [...study.concepts] : [];
  }

  function defaultProgress() {
    return { answers: {}, missed: {}, flagged: {}, history: [], study: { chapters: {} }, diagnostics: {} };
  }
  ...
  function mergeProgress(localValue, cloudValue) {
    ...
    merged.diagnostics = { ...cloud.diagnostics, ...local.diagnostics };
    return merged;
  }
  ...
  function sessionKey() {
    return `${SESSION_KEY}:${activeCertificationId()}`;
  }
  ...
  function progressKey() {
    return authUser?.id ? `${PROGRESS_KEY}:${activeCertificationId()}:${authUser.id}` : `${PROGRESS_KEY}:${activeCertificationId()}:guest`;
  }

  function loadStoredProgress() {
    const legacy = isTexasTrack() ? loadJson(legacyProgressKey(), defaultProgress()) : defaultProgress();
    progress = normalizeProgress(loadJson(progressKey(), legacy));
  }

  function storedProgressForTrack(trackId) {
    const fallback = trackId === "texas-life" ? loadJson(legacyProgressKey(), defaultProgress()) : defaultProgress();
    return normalizeProgress(loadJson(authUser?.id ? `${PROGRESS_KEY}:${trackId}:${authUser.id}` : `${PROGRESS_KEY}:${trackId}:guest`, fallback));
  }
  ...
  function saveProgress(sync = true) {
    saveJson(progressKey(), normalizeProgress(progress));
  ...
  function normalizeQuestion(question) {
    if (QuestionEngine.normalizeQuestion) {
      return QuestionEngine.normalizeQuestion(question, { language: prefs.language, certificationId: activeCertificationId() });
    }
    return question;
  }
  ...
  function normalizeQuestionBank(questions) {
    ...
    if (QuestionEngine.normalizeQuestions) {
      return QuestionEngine.normalizeQuestions(source, { language: prefs.language, certificationId: activeCertificationId() });
    }
    return source;
  }

  function questionPracticeLanguage() {
    return isTexasTrack() ? prefs.language : "en";
  }

  function localizedQuestionContent(question) {
    const normalized = normalizeQuestion(question);
    const language = questionPracticeLanguage();
    const copy = QuestionEngine.contentFor ? QuestionEngine.contentFor(normalized, language) : normalized[language];
    return copy?.question ? copy : normalized.en || normalized.es || { question: "", answers: [] };
  }

  function localizedAnswers(question) {
    const normalized = normalizeQuestion(question);
    const answers = QuestionEngine.answerList ? QuestionEngine.answerList(normalized, questionPracticeLanguage()) : localizedQuestionContent(normalized).answers;
  ...
  function topicLabel(topic) {
    const domain = activeTrack().domains?.find((item) => item.id === topic);
    if (domain) return domain.title || domain.shortTitle || topic;
    const labels = {
      es: {
  ...
  function updateBankPill() {
    const track = activeTrack();
    const count = track.questionTarget || questionBank.length;
    els.bankPill.textContent = `${count} ${prefs.language === "es" ? "preguntas" : "questions"}`;
  }

  function trackDescription(track) {
    if (track.id === "bcsp-cit") return t("trackCitDesc");
    return t("trackTexasDesc");
  }

  function trackSnapshot(track) {
    const trackProgress = storedProgressForTrack(track.id);
    const questions = QuestionEngine.normalizeQuestions
      ? QuestionEngine.normalizeQuestions(questionSourceForTrack(track), { language: prefs.language, certificationId: track.id })
      : uniqueQuestions(questionSourceForTrack(track));
    const model = readinessModel(trackProgress, questions, track);
    const lastSession = trackProgress.history?.[0];
    const readiness = model.hasBaseline ? `${model.readiness}%` : model.confidenceLabel;
    const lastScore = lastSession ? `${lastSession.percent}% · ${lastSession.correct}/${lastSession.total}` : t("noAttemptsYet");
    return `
      <div class="track-snapshot" aria-label="${escapeHtml(track.shortLabel || track.label || track.id)} ${escapeHtml(t("results"))}">
        <span><b>${escapeHtml(t("readiness"))}</b>${escapeHtml(readiness)}</span>
        <span><b>${escapeHtml(t("answered"))}</b>${model.answered}</span>
        <span><b>${escapeHtml(t("lastScore"))}</b>${escapeHtml(lastScore)}</span>
      </div>
    `;
  }

  function renderTrackSwitcher() {
    if (!els.certificationTrackGrid) return;
    const activeId = activeCertificationId();
    const tracks = Object.values(TRACKS).length ? Object.values(TRACKS) : [activeTrack()];
    els.activeTrackBadge.textContent = activeTrack().shortLabel || activeTrack().label || activeId;
    els.certificationTrackGrid.innerHTML = tracks.map((track) => {
      const isActive = track.id === activeId;
      const trackQuestionCount = QuestionEngine.normalizeQuestions
        ? QuestionEngine.normalizeQuestions(questionSourceForTrack(track), { language: "en", certificationId: track.id }).length
        : uniqueQuestions(questionSourceForTrack(track)).length;
      const officialFormatDetail = prefs.language === "es"
        ? `${track.examQuestionCount || 100} ${t("officialQuestionFormat")}`
        : `${track.examQuestionCount || 100}-${t("officialQuestionFormat")}`;
      const detail = track.blueprint
        ? `${trackQuestionCount} ${t("practiceQuestions")} · ${officialFormatDetail}`
        : `${trackQuestionCount || track.questionTarget || questionBank.length} ${prefs.language === "es" ? "preguntas" : "questions"}`;
      return `
        <button class="track-card ${isActive ? "active" : ""}" type="button" data-track-id="${escapeHtml(track.id)}" aria-pressed="${isActive}">
          <span>${escapeHtml(isActive ? t("activeTrack") : track.organization || t("currentTrack"))}</span>
          <strong>${escapeHtml(track.label || track.shortLabel || track.id)}</strong>
          <p>${escapeHtml(trackDescription(track))}</p>
          <small>${escapeHtml(detail)}</small>
          ${trackSnapshot(track)}
        </button>
      `;
    }).join("");
    els.certificationTrackGrid.querySelectorAll("[data-track-id]").forEach((button) => {
      button.addEventListener("click", () => switchCertification(button.dataset.trackId));
    });
  }

  async function switchCertification(certificationId) {
    if (!TRACKS[certificationId] || certificationId === activeCertificationId()) return;
    saveProgress(false);
    prefs.certificationId = certificationId;
    const track = activeTrack();
    if (track.defaultLanguage && track.defaultLanguage !== prefs.language && track.bilingual !== true) {
      prefs.language = track.defaultLanguage;
    }
    saveJson(PREF_KEY, prefs);
    loadStoredProgress();
    session = loadJson(sessionKey(), null);
    questionBank = normalizeQuestionBank(localQuestionSource());
    flashcardBank = localFlashcardSource();
    flashcardIndex = 0;
    flashcardChapterFilter = "all";
    flashcardTypeFilter = "all";
    if (isTexasTrack()) {
  ...
  async function loadCloudProgress() {
    ...
    const { data, error } = await supabaseClient
      ...
      .maybeSingle();
    if (error) return;
    const cloudPayload = data?.progress || defaultProgress();
    const cloudTrack = cloudPayload.tracks?.[activeCertificationId()] || (isTexasTrack() ? cloudPayload : defaultProgress());
    progress = mergeProgress(progress, cloudTrack);
    saveProgress(false);
    renderSessionSummary();
    updateDashboard();
    if (activeScreen === "progress") renderProgress();
  ...
  async function syncProgressToCloud() {
    ...
    const { data } = await supabaseClient
      ...
      .eq("user_id", authUser.id)
      .maybeSingle();
    const current = data?.progress && typeof data.progress === "object" ? data.progress : {};
    const payload = current.tracks ? current : { tracks: isTexasTrack() && current.answers ? { "texas-life": normalizeProgress(current) } : {} };
    payload.tracks = payload.tracks || {};
    payload.tracks[activeCertificationId()] = normalizeProgress(progress);
  ...
  function populateFilters() {
    ...
    const previousSubtopic = els.subtopic?.value || "all";
    const previousDifficulty = els.difficulty?.value || "all";
    const previousQuestionType = els.questionType?.value || "all";
    ...
    if (els.topic?.previousElementSibling) els.topic.previousElementSibling.textContent = isTexasTrack() ? t("topic") : t("domain");
    fillSelect(els.topic, [{ value: "all", label: isTexasTrack() ? t("allTopics") : t("allDomains") }, { value: "missed", label: t("reviewMissed") }, ...topics.map((topic) => ({ value: topic, label: topicLabel(topic) }))], previousTopic);
    ...
    els.simulator?.closest("label")?.classList.toggle("hidden", !isTexasTrack());
    const showCitFilters = !isTexasTrack();
    els.subtopicFilterWrap?.classList.toggle("hidden", !showCitFilters);
    els.difficultyFilterWrap?.classList.toggle("hidden", !showCitFilters);
    els.questionTypeFilterWrap?.classList.toggle("hidden", !showCitFilters);
    if (showCitFilters) {
      const subtopics = [...new Set(questionBank.map((question) => question.subtopic || question.metadata?.subtopicId).filter(Boolean))].sort();
      const difficulties = [...new Set(questionBank.map((question) => question.difficultyLevel || question.metadata?.difficultyLabel).filter(Boolean))].sort();
      const questionTypes = [...new Set(questionBank.map((question) => question.questionType || question.metadata?.style).filter(Boolean))].sort();
      fillSelect(els.subtopic, [{ value: "all", label: t("allSubtopics") }, ...subtopics.map((subtopic) => ({ value: subtopic, label: titleCase(subtopic) }))], previousSubtopic);
      fillSelect(els.difficulty, [{ value: "all", label: t("allDifficulties") }, ...difficulties.map((difficulty) => ({ value: difficulty, label: difficulty }))], previousDifficulty);
      fillSelect(els.questionType, [{ value: "all", label: t("allQuestionTypes") }, ...questionTypes.map((type) => ({ value: type, label: type }))], previousQuestionType);
    }
    populateCounts();
  }
  ...
  function populateStudyChapters() {
    const study = activeStudy();
    if (!els.studyChapterSelect || !study?.chapters?.length) return;
    ...
    fillSelect(
      ...
      selected || study.chapters[0].id
    );
  }

  function currentStudyChapter() {
    const study = activeStudy();
    return study?.chapters?.find((chapter) => chapter.id === els.studyChapterSelect.value) || study?.chapters?.[0];
  }

  function topicForChapter(number) {
    if (!isTexasTrack()) {
      const chapter = activeStudy()?.chapters?.find((item) => Number(item.number) === Number(number));
      return chapter?.id || activeTrack().domains?.find((domain) => Number(domain.number) === Number(number))?.id || "cit-d1";
    }
    return {
      1: "general",
      2: "general",
  ...
  function chapterForTopic(topic) {
    const study = activeStudy();
    const chapter = study?.chapters?.find((item) => topicForChapter(item.number) === topic || item.id === topic);
    return chapter?.id || study?.chapters?.[0]?.id || "";
  }

  function chapterCheatCode(chapter) {
    if (!isTexasTrack()) {
      const title = chapter?.title?.en || chapter?.title?.[prefs.language] || "this CIT domain";
      const domain = activeTrack().domains?.find((item) => item.id === chapter?.id);
      return [
        `${title} is about choosing the training move that solves the real learning or performance problem, not the answer that merely sounds professional.`,
        domain?.supplemental
          ? "Ethics items are not weighted as a CIT2 exam domain here, but they protect credential credibility and professional judgment."
          : `This domain is weighted at ${domain?.weight || 0}% in the CIT2 blueprint, so practice it as a working trainer would apply it.`,
        "Read the domain summary, drill the sample questions, then add reviewed question batches as the bank grows."
      ];
    }
  ...
  function renderChapterProgress(chapter) {
    const study = activeStudy();
    if (!els.studyChapterProgress || !study?.chapters?.length) return;
  ...
  function renderStudyTerms(chapterNumber) {
    ...
    const terms = (activeStudy()?.concepts || []).filter((concept) => Number(concept.chapter) === Number(chapterNumber)).slice(0, 24);
    if (!terms.length) {
  ...
  function allFlashcards() {
    return (flashcardBank || []).map((concept, index) => {
      ...
      const chapter = activeStudy()?.chapters?.find((item) => Number(item.number) === Number(concept.chapter));
      return {
        ...normalized,
  ...
  function filteredQuestions() {
    return uniqueQuestions(questionBank.filter((question) => {
      ...
      const simulatorMatch = !isTexasTrack() || els.simulator.value === "all" || els.simulator.value === "" || String(question.simulator) === String(els.simulator.value);
      const subtopicValue = els.subtopic?.value || "all";
      const difficultyValue = els.difficulty?.value || "all";
      const typeValue = els.questionType?.value || "all";
      const subtopicMatch = isTexasTrack() || subtopicValue === "all" || question.subtopic === subtopicValue || question.metadata?.subtopicId === subtopicValue;
      const difficultyMatch = isTexasTrack() || difficultyValue === "all" || question.difficultyLevel === difficultyValue || question.metadata?.difficultyLabel === difficultyValue;
      const typeMatch = isTexasTrack() || typeValue === "all" || question.questionType === typeValue || question.metadata?.style === typeValue;
      return topicMatch && simulatorMatch && subtopicMatch && difficultyMatch && typeMatch;
    }));
  }

  function openSetup(mode) {
    els.mode.value = ["exam", "diagnostic", "forge"].includes(mode) ? mode : "practice";
    els.topic.value = mode === "missed" ? "missed" : "all";
    els.setupTitle.textContent = mode === "exam" ? t("examSimulation") : mode === "diagnostic" ? t("coldDiagnostic") : mode === "forge" ? t("forgePractice") : mode === "missed" ? t("reviewMissed") : t("setupTitle");
    populateCounts();
    showScreen("setup");
  }
  ...
  function buildDeck() {
    ...
    const engineSession = QuestionEngine.buildQuestionSession
      ? QuestionEngine.buildQuestionSession(pool, {
        ...
        mode: els.mode.value,
        language: questionPracticeLanguage(),
        certificationId: activeCertificationId(),
        topic: els.topic.value,
        simulator: els.simulator.value,
        domainWeights: activeTrack().domains,
        progress
      })
  ...
  function renderSessionSummary() {
    const saved = loadJson(sessionKey(), null);
    if (!saved?.deck?.length) {
      const languageNote = !isTexasTrack() && prefs.language === "es" ? ` ${t("citEnglishFirst")}` : "";
      renderMessage(els.sessionSummary, `${t("noSession")} ${t("balancedMix")}${languageNote}`);
      els.resume.disabled = true;
      return;
    }
    els.resume.disabled = false;
    const languageNote = !isTexasTrack() && prefs.language === "es" ? ` ${t("citEnglishFirst")}` : "";
    renderMessage(els.sessionSummary, `${t("sessionSaved")}: ${saved.index + 1}/${saved.deck.length} · ${saved.mode}${languageNote}`);
  }
  ...
  function renderQuestion() {
    ...
    const sourceLabel = isTexasTrack()
      ? `${t("simulator")} ${question.simulator} · ${question.id.toUpperCase()}`
      : `${t("domain")} ${topicLabel(question.topic || question.domain)} · ${question.id.toUpperCase()}`;
    els.questionSource.innerHTML = `
      <span>${escapeHtml(sourceLabel)}${session.reviewingMissed ? ` · ${escapeHtml(t("reviewingMissed"))}` : ""}</span>
  ...
  function buildInstructorExplanation(question) {
    const answers = localizedAnswers(question);
  ...
  function instructorWhyCorrect(question, concept) {
    ...
    const stored = cleanInstructorText(localizedQuestionContent(question).explanation || "");
    if (stored.length > 70) return stored;
    const correct = localizedAnswers(question).find((answer) => answer.id === question.correctAnswerId);
    const correctText = correct?.text || "";
    if (prefs.language === "es") {
      return `Esta pregunta está evaluando el concepto "${correctText}". Define ese término en palabras simples y compáralo con las otras opciones. Varias respuestas pueden sonar familiares, pero solo una describe exactamente la regla que se está usando.`;
    }
    return `This question is testing the concept "${correctText}". Define that term in plain English, then compare it against the other choices. Several answers may sound familiar, but only one describes the exact insurance rule being used.`;
  }
  ...
  const DISTRACTOR_EXPLANATIONS = {
    en: [
      ...
      { match: /valued/, text: "valued means the policy pays a stated amount instead of measuring the exact dollar loss at death." },
      { match: /indemnity/, text: "indemnity means reimbursing an actual financial loss, which fits property insurance better than life insurance." },
      { match: /speculative/, text: "speculative risk includes a chance of gain, and insurance is designed for pure risk instead." },
      { match: /bilateral/, text: "bilateral means both parties make enforceable promises; life insurance is usually tested as unilateral." },
    ...
    es: [
      ...
      { match: /valorado|valued|valor determinado/, text: "contrato valorado significa que la póliza paga una cantidad establecida, no una pérdida exacta calculada al morir." },
      { match: /indemnización|indemnity/, text: "indemnización significa reembolsar una pérdida económica real, idea que encaja mejor con seguros de propiedad que con vida." },
      { match: /especulativo|speculative/, text: "riesgo especulativo incluye posibilidad de ganancia; el seguro cubre riesgo puro, no especulativo." },
      { match: /bilateral/, text: "bilateral significa que ambas partes hacen promesas exigibles; seguro de vida normalmente se prueba como unilateral." },
  ...
  function conceptLesson(question, correctText) {
    ...
    const lessons = [
      {
        match: /life insurance is an example|valued contract|\bvalued\b|of indemnity|contract that is/,
        en: {
          notice: "Life insurance is tested as a valued contract.",
          plain: "A valued contract pays the amount stated in the policy when the covered event happens. Life insurance does not wait until death and then calculate the family's exact financial loss dollar by dollar.",
          correct: "The death benefit is set when the policy is issued, so the policy pays that agreed value if the insured dies while coverage is in force. That is why life insurance is treated as valued rather than indemnity.",
          wrong: "Speculative means there is a chance of gain, and insurance is not built for speculative risk. Indemnity means reimbursing the exact amount of loss, which is the property-insurance idea. Bilateral means both sides make enforceable promises, while life insurance is normally tested as unilateral because the insurer is the party making the enforceable promise.",
          rule: "Life insurance = valued contract. Property insurance = indemnity.",
          example: "If a $250,000 life policy is active when the insured dies, the insurer pays $250,000; it does not ask the beneficiary to prove the death caused exactly $250,000 of loss."
        },
        es: {
          notice: "Seguro de vida se prueba como contrato valorado.",
          plain: "Un contrato valorado paga la cantidad establecida en la póliza cuando ocurre el evento cubierto. El seguro de vida no espera hasta la muerte para calcular dólar por dólar la pérdida económica exacta de la familia.",
          correct: "El beneficio por muerte se fija cuando se emite la póliza, así que paga ese valor acordado si el asegurado muere mientras la cobertura está vigente. Por eso seguro de vida se trata como valorado, no como indemnización.",
          wrong: "Especulativo significa que puede haber ganancia, y el seguro no está diseñado para riesgo especulativo. Indemnización significa reembolsar la pérdida exacta, idea típica de seguros de propiedad. Bilateral significa que ambas partes hacen promesas exigibles, mientras seguro de vida normalmente se prueba como unilateral porque la aseguradora hace la promesa exigible.",
          rule: "Seguro de vida = contrato valorado. Seguro de propiedad = indemnización.",
          example: "Si una póliza de vida de $250,000 está vigente cuando muere el asegurado, la aseguradora paga $250,000; no exige probar que la muerte causó exactamente $250,000 de pérdida."
        }
      },
      {
        match: /ambiguities|ambigüedades|interpreted in favor|favor of the insured|contract of adhesion|adhesion/,
        en: {
          notice: "Ambiguity favoring the insured points to adhesion.",
          plain: "An insurance policy is a contract of adhesion because the insurer writes the policy language and the applicant usually accepts it as written. Since the insurer controls the wording, unclear language is interpreted against the insurer and in favor of the insured.",
          correct: "This protects the insured from being hurt by confusing wording they did not draft. The idea is not that the insured gets every dispute automatically, but that unclear policy language is read in the insured's favor.",
          wrong: "Conditional means benefits depend on policy conditions being met. Aleatory means unequal exchange of value. Unilateral means only the insurer makes the enforceable promise after premiums are paid.",
          rule: "Insurer writes it. Ambiguity favors the insured.",
          example: "If a policy phrase can reasonably mean two things, the interpretation that helps the insured is usually used because the insurer wrote the contract."
        },
        es: {
          notice: "Ambigüedad a favor del asegurado apunta a adhesión.",
          plain: "Una póliza de seguro es un contrato de adhesión porque la aseguradora redacta el lenguaje y el solicitante normalmente lo acepta como está escrito. Como la aseguradora controla las palabras, el lenguaje poco claro se interpreta contra la aseguradora y a favor del asegurado.",
          correct: "Esto protege al asegurado de ser perjudicado por palabras confusas que no redactó. No significa que el asegurado gana toda disputa automáticamente; significa que las ambigüedades razonables favorecen al asegurado.",
          wrong: "Condicional significa que los beneficios dependen de cumplir condiciones. Aleatorio significa intercambio desigual de valor. Unilateral significa que solo la aseguradora hace la promesa exigible después de pagar primas.",
          rule: "La aseguradora lo redacta. La ambigüedad favorece al asegurado.",
          example: "Si una frase de la póliza puede significar dos cosas razonables, normalmente se usa la interpretación que ayuda al asegurado porque la aseguradora redactó el contrato."
        }
      },
      {
        match: /pure risk|speculative risk|chance of gain|risk is insurable|hazard|peril/,
        en: {
          notice: "Insurance covers pure risk, not speculative risk.",
          plain: "Pure risk means there is a chance of loss or no loss, but no chance of gain. Speculative risk has the possibility of gain, like investing or gambling, so it is not the kind of risk life insurance is designed to cover.",
          correct: "Life insurance transfers the pure risk of premature death from the family to the insurer. It is not meant to create profit from a gamble.",
          wrong: "A peril is the cause of loss, such as death or fire. A hazard is something that increases the chance or severity of loss. Indemnity is a reimbursement concept, not the basic type of risk being insured.",
          rule: "Pure risk can be insured. Speculative risk cannot.",
          example: "Dying too soon is a pure risk. Buying a stock hoping it goes up is speculative risk."
        },
        es: {
          notice: "El seguro cubre riesgo puro, no riesgo especulativo.",
          plain: "Riesgo puro significa que puede haber pérdida o no pérdida, pero no ganancia. Riesgo especulativo tiene posibilidad de ganancia, como invertir o apostar, por eso no es el tipo de riesgo que cubre el seguro de vida.",
          correct: "El seguro de vida transfiere el riesgo puro de muerte prematura de la familia a la aseguradora. No está diseñado para crear ganancia por una apuesta.",
          wrong: "Un peril/peligro es la causa de la pérdida, como muerte o fuego. Un hazard aumenta la probabilidad o severidad de pérdida. Indemnización es una idea de reembolso, no el tipo básico de riesgo asegurado.",
          rule: "Riesgo puro se puede asegurar. Riesgo especulativo no.",
          example: "Morir demasiado pronto es riesgo puro. Comprar una acción esperando que suba es riesgo especulativo."
        }
      },
      {
  ...
  function finishSession() {
    if (session?.index < (session?.deck?.length || 0) - 1 && !window.confirm(t("finishEarlyConfirm"))) return;
    const missed = [];
    let correct = 0;
    ...
    if (!isTrialSession()) {
      if (session.mode === "diagnostic") {
        progress.diagnostics = progress.diagnostics || {};
        progress.diagnostics.cold = {
          date: new Date().toISOString(),
          certificationId: activeCertificationId(),
          correct,
          total,
          percent,
          domainCounts: session.diagnostics?.domainCounts || {}
        };
      }
      progress.history.unshift({ date: new Date().toISOString(), certificationId: activeCertificationId(), mode: session.mode, language: prefs.language, correct, total, percent });
  ...
  function renderResults() {
    ...
    const isReadinessSample = session.mode === "exam" && score.total >= 50;
    document.getElementById("practiceAgainButton").textContent = t("practiceAgain");
    els.topicBreakdownTitle.textContent = isReadinessSample ? t("topicBreakdown") : t("setBreakdown");
    ...
    els.resultTitle.textContent = resultTitleFor(score.percent, isReadinessSample);
    els.resultSummary.textContent = `${t("youScored")}: ${score.correct} / ${score.total}${isReadinessSample ? "" : ` · ${t("limitedPracticeSet")}`}`;
    renderResultInsight();
    els.reviewLastMissed.classList.toggle("hidden", score.missed.length === 0);
    renderTopicBreakdown(isReadinessSample);
  }

  function resultTitleFor(percent, isReadinessSample) {
    if (isReadinessSample) {
      if (percent >= 84) return t("strong");
      if (percent >= 70) return t("passing");
      return t("keepPracticing");
    }
    if (percent >= 84) return t("strongPracticeSet");
    if (percent >= 70) return t("practiceSetComplete");
    return t("keepBuilding");
  }
  ...
  function sampleNote(value, topic = "") {
    const notes = [];
    if (value.total < 5) notes.push(t("limitedSample"));
    if (activeTrack().domains?.find((domain) => domain.id === topic)?.supplemental) notes.push(t("supplementalTopic"));
    return notes.length ? `<small>${escapeHtml(notes.join(" · "))}</small>` : "";
  }
  ...
  function renderStyleBreakdown(analytics) {
    ...
    Object.entries(analytics.byStyle)
      ...
      .forEach(([style, value]) => {
        ...
        row.innerHTML = `<div class="topic-top"><span>${escapeHtml(styleLabel(style))}${sampleNote(value)}</span><span>${value.correct}/${value.total} · ${percent}%</span></div><div class="progress-track"><div class="progress-fill" style="width:${percent}%"></div></div>`;
        els.topicBreakdown.appendChild(row);
      });
  }

  function renderTopicBreakdown(isReadinessSample = false) {
    const byTopic = {};
    session.deck.forEach((deckItem) => {
      const question = qById(deckItem.id);
    ...
    if (!isReadinessSample) {
      ...
      note.className = "muted breakdown-note";
      note.textContent = t("limitedPracticeSet");
      els.topicBreakdown.appendChild(note);
    }
    Object.entries(byTopic).sort(([a], [b]) => a.localeCompare(b)).forEach(([topic, value]) => {
      const percent = Math.round((value.correct / value.total) * 100);
      const row = document.createElement("div");
      row.className = "topic-row";
      row.innerHTML = `<div class="topic-top"><span>${topicLabel(topic)}${sampleNote(value, topic)}</span><span>${value.correct}/${value.total} · ${percent}%</span></div><div class="progress-track"><div class="progress-fill" style="width:${percent}%"></div></div>`;
      els.topicBreakdown.appendChild(row);
    });
  ...
  function totalAnswered(progressSource = progress) {
    return Object.values(progressSource.answers || {}).reduce((sum, record) => sum + (Number(record.seen) || 0), 0);
  }

  function topicStats(progressSource = progress, questions = questionBank, track = activeTrack()) {
    const stats = {};
    questions.forEach((question) => {
      stats[question.topic] ||= { topic: question.topic, seen: 0, correct: 0, wrong: 0, missed: 0, total: 0 };
      stats[question.topic].total += 1;
      const record = progressSource.answers?.[question.id];
      if (record) {
      ...
      if (progressSource.missed?.[question.id]) stats[question.topic].missed += 1;
    });
  ...
  function studyStreak(progressSource = progress) {
    const days = new Set((progressSource.history || []).map((entry) => new Date(entry.date).toDateString()));
  ...
  function recentAccuracy(progressSource = progress) {
    const recent = (progressSource.history || []).slice(0, 5);
  ...
  function examHistory(progressSource = progress) {
    return (progressSource.history || []).filter((entry) => entry.mode === "exam" && Number(entry.total) >= 50);
  }

  function weightedExamScore(progressSource = progress) {
    const exams = examHistory(progressSource).slice(0, 3);
  ...
  function readinessModel(progressSource = progress, questions = questionBank, track = activeTrack()) {
    const records = Object.values(progressSource.answers || {});
    ...
    const stats = topicStats(progressSource, questions, track);
    const weightedStats = stats.filter((item) => !track.domains?.find((domain) => domain.id === item.topic)?.supplemental);
    const scoredStats = weightedStats.length ? weightedStats : stats;
    const mastered = scoredStats.filter((item) => item.seen >= 5 && item.accuracy >= 82 && item.missed === 0).length;
    const masteryPercent = scoredStats.length ? Math.round((mastered / scoredStats.length) * 100) : 0;
    const topicCoverage = scoredStats.length ? Math.round((scoredStats.filter((item) => item.seen >= 3).length / scoredStats.length) * 100) : 0;
    const recent = recentAccuracy(progressSource);
    const examScore = weightedExamScore(progressSource);
    const examsTaken = examHistory(progressSource).length;
    const fullExamTaken = (progressSource.history || []).some((entry) => entry.mode === "exam" && Number(entry.total) >= 100);
    const seenStyles = new Set();
    const seenDifficulties = new Set();
    Object.entries(progressSource.answers || {}).forEach(([id, record]) => {
      if (!Number(record?.seen)) return;
      const question = questions.find((item) => item.id === id);
      if (!question) return;
      seenStyles.add(question.metadata?.style || question.questionType || "recall");
      seenDifficulties.add(String(question.metadata?.difficulty || question.difficulty || 1));
    });
    const expectedStyles = new Set(questions.map((question) => question.metadata?.style || question.questionType).filter(Boolean));
    const expectedDifficulties = new Set(questions.map((question) => String(question.metadata?.difficulty || question.difficulty || "")).filter(Boolean));
    const styleCoverage = expectedStyles.size ? Math.round((seenStyles.size / expectedStyles.size) * 100) : 0;
    const difficultyCoverage = expectedDifficulties.size ? Math.round((seenDifficulties.size / expectedDifficulties.size) * 100) : 0;
    const weakPenalty = scoredStats.reduce((sum, item) => {
      if (!item.seen) return sum + 4;
      if (item.accuracy < 65) return sum + 8;
      if (item.accuracy < 75) return sum + 4;
      return sum;
    }, 0);
    const consistency = clamp(studyStreak() * 12, 0, 100);
    const hasBaseline = answered >= 50 || examsTaken > 0;
    const readiness = hasBaseline
      ? Math.round(
        (examScore ?? accuracy) * 0.32
        + (recent ?? accuracy) * 0.18
        + accuracy * 0.14
        + masteryPercent * 0.12
        + topicCoverage * 0.12
        + styleCoverage * 0.05
        + difficultyCoverage * 0.04
        + consistency * 0.03
        - weakPenalty
      )
      : null;
    ...
    const evidenceScore = clamp(Math.round(answered * 0.28 + topicCoverage * 0.22 + styleCoverage * 0.16 + difficultyCoverage * 0.14 + Math.min(examsTaken, 3) * 10 + (fullExamTaken ? 12 : 0)), 0, 100);
    const confidence = evidenceScore >= 80 ? "high" : evidenceScore >= 55 ? "medium" : "low";
    const confidenceLabel = confidence === "high" ? t("highConfidence") : confidence === "medium" ? t("mediumConfidence") : t("lowConfidence");
    const label = !hasBaseline ? t("notEnoughData") : safeReadiness >= 84 && confidence !== "low" ? t("examReady") : safeReadiness >= 70 ? t("nearlyReady") : safeReadiness >= 45 ? t("improving") : t("needsWork");
    const passingChance = !hasBaseline || confidence === "low" ? null : clamp(Math.round(safeReadiness + (examScore !== null ? 3 : -7) + (confidence === "high" ? 3 : 0)), 10, 95);
    return {
      answered,
      correct,
      accuracy,
      ...
      passingChance,
      studyHours: hasBaseline ? clamp(Math.ceil((100 - safeReadiness) / 7), 1, 20) : 6,
      streak: studyStreak(progressSource),
      mastered,
      weakest,
      label,
      hasBaseline,
  ...
  function renderMissionControl() {
    ...
    const passingLabel = document.querySelector("[data-i18n='estimatedPassChance']");
    const showPassChance = isTexasTrack() && model.passingChance !== null;
    if (passingLabel) passingLabel.textContent = !isTexasTrack() ? t("readinessEstimate") : showPassChance ? t("estimatedPassChance") : t("readinessConfidence");
    ...
    els.missionPassingChance.textContent = !isTexasTrack() && model.hasBaseline ? `${model.readiness}%` : showPassChance ? `${model.passingChance}%` : model.confidenceLabel;
    els.missionPassingChance.classList.toggle("is-text", !showPassChance && (!model.hasBaseline || isTexasTrack()));
    if (els.missionReadinessNote) els.missionReadinessNote.textContent = isTexasTrack() ? t("readinessNoteTexas") : t("readinessNoteCit");
  ...
  function openDiagnostic() {
    openSetup(!isTexasTrack() ? "diagnostic" : "practice");
    els.topic.value = "all";
    if (els.simulator) els.simulator.value = "all";
    populateCounts();
    const target = !isTexasTrack() ? "50" : "25";
    if ([...els.count.options].some((option) => option.value === target)) els.count.value = target;
  }
  ...
  function openWeakTopicStudy(topic) {
    ...
    const chapter = activeStudy()?.chapters?.find((item) => Number(item.number) === Number(chapterNumber));
  ...
  const WEAKNESS_COACHING = {
    ...
    es: {
      ...
      general: {
        ...
        fix: "Repasa la lección base y luego haz un diagnóstico de 50 preguntas."
      },
      "life insurance": {
  ...
  function bindEvents() {
    ...
    els.subtopic?.addEventListener("change", populateCounts);
    els.difficulty?.addEventListener("change", populateCounts);
    els.questionType?.addEventListener("change", populateCounts);
    els.mode?.addEventListener("change", populateCounts);
